...
function main(pr)
    sumoExe = SUMO
    traffic_config = "../SUMOSim/AnnArbor/urban/test_oneshot_$(pr).sumocfg"
    config_info = Dict("summary"=>"summary_oneshot$(pr).xml",
                       "tripinfo"=>"tripinfo_oneshot$(pr).xml",
                       "vehroute"=>"routeinfo_oneshot$(pr).xml")
    ...
    traci.close()
    CSV.write("urban_oneshot_$(pr).csv", df)
end
