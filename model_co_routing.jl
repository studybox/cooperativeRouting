...
function doStep!(traffic::SimpleUrbanNetwork, all_fleets::AllFleet, rng::AbstractRNG)
    ...
    for (vehid, sub) in subscribes
        ...
        update!(all_fleets, vehid, fromTAZ, toTAZ, time/60)
    end
    ...
    for fleet in fleets_to_update
        ...
        mdp.current_time = time/60
        ac = action(fleet.planner, fleet.current_state)
        if ac.route != fleet.current_action.route
            fleet.current_action = ac
            # update route
            ac_ind = findfirst(x->x==fleet.current_state.name, fleet.route)
            fleet.route = [fleet.route[1:ac_ind];ac.route]
...
function main(pr)
    ...
    traffic_config = "../SUMOSim/AnnArbor/urban/test_model_decmcts_$(pr).sumocfg"
    
    config_info = Dict("summary"=>"summary_model_decmcts$(pr).xml",
                       "tripinfo"=>"tripinfo_model_decmcts$(pr).xml",
                       "vehroute"=>"routeinfo_model_decmcts$(pr).xml",
                       "teleport"=>"-1",
    ...
    traci.close()
    CSV.write("urban_model_dec_$(pr).csv", df)
end
